export async function onRequestGet(context) {
  ...
  function calculateScore(saveRate, avgWatchSec, plays, intentCount, totalComments) {
    // 留言購買意圖 45分
    let intentScore = 0;
    ...
    return {
      total: Math.min(100, intentScore + saveScore + watchScore + playsScore),
      intent: intentScore,
      save: saveScore,
      watch: watchScore,
      plays: playsScore
  ...
  try {
    ...
    const reels = await Promise.all(videoMedia.slice(0, 15).map(async (m) => {
      ...
      const scoreData = calculateScore(saveRate, watchSec, plays, intentCount, totalComments);
      ...
      return {
        ...
        score_breakdown: { intent: scoreData.intent, save: scoreData.save, watch: scoreData.watch, plays: scoreData.plays },
    ...
    return new Response(JSON.stringify({
      ...
      debug: { total_media: allMedia.length, video_count: videoMedia.length,
        pages_error: pagesData.error || null, media_error: mediaData.error || null }
